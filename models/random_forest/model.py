...
CROSS_VALIDATION_RESULTS_OUTPUT_PATH = (
    PROJECT_ROOT
    / "artifacts"
    / "metrics"
...
def save_outputs(
    ...
    FEATURE_RANKING_OUTPUT_PATH.parent.mkdir(
        parents=True,
        exist_ok=True,
    )
...
def main() -> None:
    ...
    print(
        f"  Feature ranking: "
        f"{FEATURE_RANKING_OUTPUT_PATH}"
    )

    print(
        f"  Selected genes: "
        f"{SELECTED_GENES_OUTPUT_PATH}"
    )

    print(
        f"  CV results: "
        f"{CROSS_VALIDATION_RESULTS_OUTPUT_PATH}"
    )
